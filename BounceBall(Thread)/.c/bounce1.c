...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COPYDATA:
		...
		value.crt = Pv->crt;
		value.flag1 = Pv->flag1;
		if (value.flag1 != 2) break;
		value.x = 300 - R;
		value.y = Pv->y;
		value.xv = Pv->xv;
		value.yv = Pv->yv;
...
void DoCreateMain(HWND hWnd)
{
	...
	value.flag2 = 0;
	value.flag3 = 0;
	value.x = 80;	//공의 x 좌표 초기값
	value.y = 250;//공의 y 좌표 초기값

	value.xv = (rand() % 4) + 5; //x벡터 속도
	value.yv = (rand() % 4) + 5; //y벡터 속도
}
...
void DoButtonMain(HWND hWnd)
{
	...
	//그 다음부터는 공의 좌표를 초기화해주고 다시 시작하겠다 라는 뜻
	if (btnflag > 0) {
		value.x = 250;
		value.y = 200;
		value.xv *= -(BOUNCE);
...
void collision(HWND hWnd)
{
	if (value.x < value.crt.left) //벗어난 경우
	{
		value.x += R;
		value.xv *= (-BOUNCE);
	}
	else if (value.x > value.crt.right+R && value.flag1 == 1) {
